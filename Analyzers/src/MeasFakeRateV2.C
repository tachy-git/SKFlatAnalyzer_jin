...
void MeasFakeRateV2::executeEventFrom(NonpromptParameter &param) {
...
void MeasFakeRateV2::ApplyScaleVariation(vector<Muon> &muons, vector<Electron> &electrons, vector<Jet> &jets, const TString &scale) {
    if (scale == "Central") {
        // do nothing
...
TString MeasFakeRateV2::SelectEvent(const vector<Muon> &muons, const vector<Muon> &vetoMuons, const vector<Electron> &electrons, const vector<Electron> &vetoElectrons, const vector<Jet> &jets) {
    TString channel = "";
...
double MeasFakeRateV2::GetEventWeight(const NonpromptParameter &param, Event &ev, vector<Muon> &muons, vector<Electron> &electrons, vector<Jet> &jets) {
    double weight = 1.;
    if (!IsDATA) {
...
double MeasFakeRateV2::GetNPVReweight(unsigned int nPV) {
...
TString MeasFakeRateV2::FindBin(const double ptcorr, const double abseta) {
    int ptcorr_idx = -1;
    int abseta_idx = -1;
...
void MeasFakeRateV2::FillObjects(const TString &channel,
