...
class EdgeConv(MessagePassing):
    def __init__(self, in_channels, out_channels):
        ...
        self.mlp = Sequential(
                Linear(2*in_channels, out_channels), ReLU(), BatchNorm1d(out_channels), Dropout(0.4),
                Linear(out_channels, out_channels), ReLU(), BatchNorm1d(out_channels), Dropout(0.4),
                Linear(out_channels, out_channels), ReLU(), BatchNorm1d(out_channels), Dropout(0.4)
                )
...
class ParticleNet(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch=None):
        ...
        x = self.bn1(x)
        x = F.dropout1d(x, p=0.4, training=self.training)
