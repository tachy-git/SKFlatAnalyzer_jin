...
class PromptEstimator(TriLeptonBase):
    ...
    def evalScore(self, tightMuons, tightElectrons, jets, bjets, METv):
        scores = {}
        if self.network == "DenseNeuralNet": 
            data = self.getDenseInput(tightMuons, tightElectrons, jets, bjets, METv)
            for sig, bkg in product(self.signalStrings, self.backgroundStrings):
                scores[f"{sig}_vs_{bkg}"] = self.getDenseScore(f"{sig}_vs_{bkg}", data)
        else:                                
            data = self.getGraphInput(tightMuons, tightElectrons, jets, METv)
            for sig, bkg in product(self.signalStrings, self.backgroundStrings):
