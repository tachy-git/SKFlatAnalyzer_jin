...
class MatrixEstimator(TriLeptonBase):
    ...
    def initializePyAnalyzer(self):
        ...
        self.systs = ["Central"]
        if self.run_syst:
            self.systs += ["PromptNormUp", "PromptNormDown", "MotherJetPtUp", "MotherJetPtDown", "RequireHeavyTag"]
    ...
    def executeEvent(self):
        ...
        for syst in self.systs:
            weight = super().getFakeWeight(looseMuons, looseElectrons, syst)
    ...
    def FillObjects(self, channel, objects, weight, syst="Central"):
        ...
        super().FillHist(f"{channel}/{syst}/METv/energy", METv.E(), weight, 300, 0., 300.)
        ...
        super().FillHist(f"{channel}/{syst}/METv/pz", METv.Pz(), weight, 500, -250., 250.)
